...
namespace OnefallGames
{
    public class AdManager : MonoBehaviour
    {
        ...
        [SerializeField]
        private float showRewardedVideoAdDelay = 1f;

        [Header("Rewarded Coins Config")]
        [SerializeField]
        private int minRewardedCoins = 20;
        [SerializeField]
        private int maxRewardedCoins = 50;
        [SerializeField]
        private float rewardDelay = 0.5f;
        ...
        private void HandleOnRewardedVideoClosed()
        {
            if (rewardedVideoAdType == AdType.Admob) //Admob ads
            {
                ...
                {
                    if (UIManager.Instance.IsWatchAdToContinue)
                    {
                        GameManager.Instance.SetContinueGame();
                    }
                    else
                    {
                        int coins = UnityEngine.Random.Range(minRewardedCoins, maxRewardedCoins) / 5 * 5;
                        GameManager.Instance.CreateCoinsForReward(coins, rewardDelay);
                    }
                }
